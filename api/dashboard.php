...
<head>
    ...
    <style>
        ...
        * { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        ...
        .stat-card:hover { transform: translateY(-5px); box-shadow: 0 20px 40px rgba(139,153,178,0.2); }
...
<body class="text-slate-800 flex min-h-screen">
    ...
    <aside class="w-20 md:w-64 bg-white border-r border-slate-100 flex flex-col items-center py-8 sticky top-0 h-screen z-50">
        ...
        <nav class="flex flex-col gap-2 w-full px-4 font-bold h-full nav-text">
            <a href="dashboard.php" class="flex items-center justify-center md:justify-start p-3 bg-blue-600 text-white rounded-xl shadow-xl shadow-blue-100">
    ...
    <main class="flex-1 p-6 md:p-10 lg:p-12 max-w-[1600px] mx-auto w-full">
        <header class="flex justify-between items-center mb-10">
            ...
            <div class="flex items-center gap-3 bg-white p-1.5 rounded-full border border-slate-100 smooth-shadow">
                <div class="flex flex-col items-end px-3">
                    <p class="text-[9px] text-slate-400 font-bold uppercase"><?php echo htmlspecialchars($role); ?></p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <?php if (in_array($role, ['Admin', 'Apoteker'])) : ?>
            <a href="racikan_obat.php" class="flex items-center justify-between bg-white p-8 rounded-[2.5rem] smooth-shadow border border-slate-50 hover:border-purple-500 group transition-all">
                <div class="flex items-center gap-6">
                    <div class="w-14 h-14 bg-purple-50 text-purple-600 rounded-xl flex items-center justify-center group-hover:bg-purple-600 group-hover:text-white transition-all">
                        <i class="fas fa-mortar-pestle text-xl"></i>
                    </div>
                    <div>
                        <h5 class="font-black text-slate-800 uppercase text-xs tracking-wide">Racikan Obat</h5>
                        <p class="text-[10px] text-slate-400 font-medium">Buat & kelola racikan.</p>
                    </div>
                </div>
                <i class="fas fa-chevron-right text-slate-200 group-hover:text-purple-500 group-hover:translate-x-1 transition-all"></i>
            </a>
            <?php endif; ?>

            <?php if (in_array($role, ['Admin', 'Manager Gudang'])) : ?>
            <a href="laporan.php" class="flex items-center justify-between bg-white p-8 rounded-[2.5rem] smooth-shadow border border-slate-50 hover:border-emerald-500 group transition-all">
                <div class="flex items-center gap-6">
                    <div class="w-14 h-14 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center group-hover:bg-emerald-600 group-hover:text-white transition-all">
                        <i class="fas fa-file-alt text-xl"></i>
                    </div>
                    <div>
                        <h5 class="font-black text-slate-800 uppercase text-xs tracking-wide">Laporan</h5>
                        <p class="text-[10px] text-slate-400 font-medium">Rekap & cetak laporan.</p>
                    </div>
                </div>
                <i class="fas fa-chevron-right text-slate-200 group-hover:text-emerald-500 group-hover:translate-x-1 transition-all"></i>
            </a>
            <?php endif; ?>
        </div>
